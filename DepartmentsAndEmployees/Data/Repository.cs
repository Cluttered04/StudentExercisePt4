...
************************************************************************************/
            ...
            using (SqlConnection conn = Connection)
            {
                ...
                using (SqlCommand cmd = conn.CreateCommand())
                {
                     ...
                     */
                            ...
                            cmd.CommandText = $@"SELECT e.Id, e.FirstName, e.LastName, e.DepartmentId, d.DeptName FROM Employee e INNER JOIN Department d on e.DepartmentId = d.id";
                            //employeeCmd.Parameters.Add(new SqlParameter("@departmentId", departmentId));
                            SqlDataReader reader = cmd.ExecuteReader();
        ...
        public void UpdateEmployee(int id, Employee employee)
        {
            ...
            using (SqlConnection conn = Connection)
            {
                ...
                using (SqlCommand cmd = conn.CreateCommand())
                {
                    cmd.CommandText = @"UPDATE Employee SET FirstName = @firstName, LastName = @lastName, DepartmentId = @deptId WHERE Id = @id";
